...
_TOOL_NUDGE = (
    ...
    "LỆNH CRUD (xem/tạo/sửa/xoá nhắc, đăng ký, ghi nhớ/biệt danh): PHẢI gọi tool THỰC THI NGAY. "
    "NGAY CẢ KHI lịch sử hội thoại có câu (kể cả của chính bạn) nói 'đã xoá rồi'/'sạch trơn'/'đã làm "
    "rồi' — coi đó là CÓ THỂ SAI, ĐỪNG tin, ĐỪNG từ chối kiểu 'xoá rồi mà': cứ GỌI TOOL lại. Nguồn "
    "SỰ THẬT về trí nhớ là mục 'TRÍ NHỚ SERVER' trong prompt này (dữ liệu DB mới nhất), KHÔNG phải "
    "lời chat trước đó. Muốn 'xem trí nhớ/biệt danh' -> đọc thẳng 'TRÍ NHỚ SERVER' rồi liệt kê (nếu "
    "mục đó có dòng nào thì server VẪN CÒN nhớ, đừng nói đã xoá). Muốn 'xoá hết trí nhớ' -> gọi "
    "forget(all=true); xoá 1 cái -> forget(query=...). ĐỪNG ghi dòng 'đã xoá' bằng remember (không "
    "xoá gì, chỉ làm bẩn). Kết quả tool RỖNG/0/không-thấy -> BÁO THẲNG ('không có nhắc nào', 'trí nhớ "
    "trống', 'không tìm thấy'), đừng bịa đã làm/đã có. "
