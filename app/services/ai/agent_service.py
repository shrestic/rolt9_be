...
_TOOL_NUDGE = (
    ...
    "LỆNH CRUD (xem/tạo/sửa/xoá nhắc, đăng ký, ghi nhớ/biệt danh): PHẢI gọi tool THỰC THI NGAY, CẤM "
    "suy đoán kết quả từ hội thoại cũ. Muốn 'xem trí nhớ/biệt danh' thì đọc mục 'TRÍ NHỚ SERVER' "
    "đã cho trong prompt (đó là dữ liệu THẬT mới nhất); muốn 'xoá hết trí nhớ' -> gọi forget(all=true), "
    "ĐỪNG ghi 1 dòng 'đã xoá' bằng remember (không xoá gì cả, chỉ làm bẩn). Kết quả tool trả về RỖNG/"
    "null/0/không-thấy -> BÁO THẲNG cho người dùng (vd 'không có nhắc nào', 'trí nhớ trống', 'không "
    "tìm thấy'), TUYỆT ĐỐI đừng bịa là đã làm/đã có. "
