...
class ClientIdMiddleware(BaseHTTPMiddleware):
    async def dispatch(self, request: Request, call_next: Callable) -> Response:
        ...
        # Skip client ID check for all auth OAuth routes (public Discord OAuth flow)
        if request.url.path.startswith("/api/v1/auth/"):
            return await call_next(request)

        # Skip client ID check for /me routes (cookie-based authenticated endpoints)
        if request.url.path.startswith("/api/v1/me/") or request.url.path == "/api/v1/me":
            return await call_next(request)

        # Skip client ID check for /guilds routes (cookie-based authenticated endpoints)
